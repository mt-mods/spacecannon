...
spacecannon.update_formspec = function(meta, is_th)
	...
	if not is_th then
		formspec = formspec .. "formspec_version[4]" .. "size[10.5,9;]"
...
spacecannon.fire = function(pos, playername, color, speed, is_th, storage_require_mod)
	...
	if is_th then
		config_store = spacecannon.config.th_powerstorage * storage_require_mod
	end
	...
	core.sound_play("spacecannon_shoot", {
		pos = pos,
		gain = 1.0,
	...
	local obj =
		core.add_entity({ x = pos.x + dir.x, y = pos.y + dir.y, z = pos.z + dir.z }, "spacecannon:energycube_" .. color)
...
spacecannon.destroy = function(pos, range, intensity)
					...
					if n and n.name ~= "air" then
						...
						if node_def.on_blast then
							...
							if resilience <= 1 or math.random(resilience) == resilience then
								core.set_node(np, { name = "air" })
								local itemstacks = core.get_node_drops(n.name)
								for _, itemname in ipairs(itemstacks) do
									if math.random(5) == 5 then
										-- chance drop
										core.add_item(np, itemname)
									end
								end
							end
						end
	...
	core.sound_play("tnt_explode", { pos = pos, gain = 1.5, max_hear_distance = math.min(radius * 20, 128) })
end
